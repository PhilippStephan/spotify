...
export class ProfileDetailsComponent implements OnInit{
  ...
  async ngOnInit(): Promise<void> {
    ...
    const topArtists = await this.requestService.getTopArtists(50, TIME_RANGE.SHORT_TERM);
    this.topArtists$.next(topArtists);
    const topTracks = await this.requestService.getTopTracks(50, TIME_RANGE.SHORT_TERM);
