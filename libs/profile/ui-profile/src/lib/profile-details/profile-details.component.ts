...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ProfileOverviewComponent, TopArtistsComponent, TopAlbumsComponent, TopTracksComponent, MatDivider, MatCard, MatCardContent],
...
export class ProfileDetailsComponent implements OnInit{
  ...
  async ngOnInit(): Promise<void> {
    ...
    console.log(user)
    this.user$.next(user)
    const followingArtists = await this.requestService.getTopArtists();
    console.log(followingArtists);
    this.followingArtists$.next(followingArtists);
    setTimeout(() => {
      this.topArtists.updateDisabled();
    }, 1000);
  }
}
